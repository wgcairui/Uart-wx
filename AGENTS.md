...
**这是硬性合规线，被驳回一次就影响上线节奏。** 所有面向 C 端用户可见的页面（登录、注册、弹窗、说明文案）必须遵守：

### 1. 不要在文案/视觉上"模仿腾讯官方"

**问题**：审核员会审查所有用户可见元素，**任何让人误以为是"微信官方页面"的设计都会被驳回**。常见误用：

- ❌ **"微信"字样**出现在 Tab/标题/按钮文案/弹窗中（"微信注册"、"使用微信绑定的手机号"、"使用微信昵称" 等）→ 改成"手机号快捷登录"、"本机手机号"、"当前账号昵称" 等中性表达
- ❌ **微信品牌绿** (`#07c160` / `#04a350` 渐变) — 那是腾讯官方色 → 换成项目主色蓝 `#487ed9 → #3158b8` 渐变
- ❌ 任何让人联想到"这是微信官方提供"的设计语言
- ❌ 代码注释里残留的"微信"可以保留（不影响审核），但**用户可见文案/颜色/Logo 全部要去掉**

**正确做法**：
- Tab 标题：「微信注册」→「手机号注册」
- 按钮文字：「使用微信绑定的手机号」→「手机号快捷登录」
- 主路径大按钮颜色：用项目主色蓝，**不要**用微信品牌绿
- 弹窗说明：用"本机手机号 / 用户头像 / 昵称"等抽象表达

**判断标准**：用户看到这页/按钮/弹窗，**不应**认为是"微信提供的小程序级原生服务"。

参考修复：本次实际被驳回的 `miniprogram/pages/login/login.{wxml,wxss,ts,ts}` 修改记录（commit `3b0a4ce` 之前的版本 vs 之后）。

### 2. 「手机号授权登录」标准措辞

**审核官方要求**（见驳回截图红框）：
- 「手机号授权登录」→ **「手机号快捷登录」**（必须用"快捷登录"）
- 衍生：所有 `open-type="getPhoneNumber"` 触发的弹窗**文案**是微信内置不可改，但前端按钮上**不要**再写"授权"二字误导用户

### 3. `getPhoneNumber` 按钮的视觉陷阱

**坑**：`open-type="getPhoneNumber"` 按钮即使文案改了，**点击还是会弹微信原生的"申请获取你的手机号"授权弹窗**（这个无法绕过，是微信提供的唯一合规拿手机号方式）。

- 不要试图用自定义弹窗**模仿**这个原生弹窗的样式
- 不要在按钮上画腾讯官方 Logo
- 颜色必须用项目主色，**不要**用微信品牌绿（这是审核最常盯的"混淆"信号）

### 4. 修改流程

1. 改 `miniprogram/pages/.../*.{wxml,wxss,ts,json}`
2. 跑 `./node_modules/typescript/bin/tsc` 重新编译
3. 开发者工具 → 编译 → 真机预览检查（注意"仿真"色板和真机有差）
4. 自查清单：
   - [ ] Tab/标题/按钮文案没有"微信"字样
   - [ ] 主路径大按钮颜色不是 `#07c160` 渐变
   - [ ] 没有任何腾讯官方 Logo / 二维码 / 引导关注公众号元素
   - [ ] `wx.getStorageSync('openid')` / `unionid` / `appid` 等**用户不可见字段**不影响审核
5. 提交发版审核
